...
begin
	...
	LatchGenericAERReg_S <= '1' when (ConfigModuleAddress_DI = GENERIC_AER_CONFIG_MODULE_ADDRESS) else '0';
	...
	begin
		...
		case ConfigParamAddress_DI is
			...
			when GENERIC_AER_CONFIG_PARAM_ADDRESSES.AckExtension_D =>
			...
			when GENERIC_AER_CONFIG_PARAM_ADDRESSES.WaitOnTransferStall_S =>
			...
			when GENERIC_AER_CONFIG_PARAM_ADDRESSES.ExternalAERControl_S =>
			...
			when GENERIC_AER_CONFIG_PARAM_ADDRESSES.HasStatistics_S =>
				if ENABLE_STATISTICS then
					GenericAEROutput_DN(0) <= '1';
				end if;

			when GENERIC_AER_CONFIG_PARAM_ADDRESSES.StatisticsEvents64_D =>
				if ENABLE_STATISTICS then
			...
			when GENERIC_AER_CONFIG_PARAM_ADDRESSES.StatisticsEvents32_D =>
				if ENABLE_STATISTICS then
					GenericAEROutput_DN(31 downto 0) <= std_logic_vector(GenericAERConfigInfoOut_DI.StatisticsEvents_D(31 downto 0));
				end if;

			when GENERIC_AER_CONFIG_PARAM_ADDRESSES.StatisticsEventsDropped64_D =>
				if ENABLE_STATISTICS then
			...
			when GENERIC_AER_CONFIG_PARAM_ADDRESSES.StatisticsEventsDropped32_D =>
				if ENABLE_STATISTICS then
	...
	begin
		if Reset_RI then                -- asynchronous reset (active high)
			...
			if LatchGenericAERReg_S and ConfigLatchInput_SI then
