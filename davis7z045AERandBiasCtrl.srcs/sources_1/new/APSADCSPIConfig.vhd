	...
	begin
		...
		case ConfigParamAddress_DI is
			...
			when APSADCCONFIG_PARAM_ADDRESSES.ResetRead_S =>
				APSADCConfigReg_DN.ResetRead_S <= ConfigParamInput_DI(0);
			...
			when APSADCCONFIG_PARAM_ADDRESSES.FrameDelay_D =>
				APSADCConfigReg_DN.FrameDelay_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.FrameDelay_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.ResetSettle_D =>
				APSADCConfigReg_DN.ResetSettle_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.ResetSettle_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.ColumnSettle_D =>
				APSADCConfigReg_DN.ColumnSettle_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.ColumnSettle_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.RowSettle_D =>
				-- Allow read/write of parameter only on boards which support it.
				if CHIP_APS_HAS_INTEGRATED_ADC = '0' then
					APSADCConfigReg_DN.RowSettle_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.RowSettle_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.NullSettle_D =>
				APSADCConfigReg_DN.NullSettle_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.NullSettle_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.StartColumn1_D =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.StartColumn1_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.StartColumn1_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.StartRow1_D =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.StartRow1_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.StartRow1_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.EndColumn1_D =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.EndColumn1_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.EndColumn1_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.EndRow1_D =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.EndRow1_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.EndRow1_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.StartColumn2_D =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.StartColumn2_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.StartColumn2_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.StartRow2_D =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.StartRow2_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.StartRow2_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.EndColumn2_D =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.EndColumn2_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.EndColumn2_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.EndRow2_D =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.EndRow2_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.EndRow2_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.StartColumn3_D =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.StartColumn3_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.StartColumn3_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.StartRow3_D =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.StartRow3_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.StartRow3_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.EndColumn3_D =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.EndColumn3_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.EndColumn3_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.EndRow3_D =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.EndRow3_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.EndRow3_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.SampleEnable_S =>
				-- Allow read/write of parameter only on chips which support it.
				if CHIP_APS_HAS_INTEGRATED_ADC = '1' then
					APSADCConfigReg_DN.SampleEnable_S <= ConfigParamInput_DI(0);
			...
			when APSADCCONFIG_PARAM_ADDRESSES.SampleSettle_D =>
				-- Allow read/write of parameter only on chips which support it.
				if CHIP_APS_HAS_INTEGRATED_ADC = '1' then
					APSADCConfigReg_DN.SampleSettle_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.SampleSettle_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.RampReset_D =>
				-- Allow read/write of parameter only on chips which support it.
				if CHIP_APS_HAS_INTEGRATED_ADC = '1' then
					APSADCConfigReg_DN.RampReset_D                   <= unsigned(ConfigParamInput_DI(tAPSADCConfig.RampReset_D'range));
			...
			when APSADCCONFIG_PARAM_ADDRESSES.RampShortReset_S =>
				-- Allow read/write of parameter only on chips which support it.
				if CHIP_APS_HAS_INTEGRATED_ADC = '1' then
					APSADCConfigReg_DN.RampShortReset_S <= ConfigParamInput_DI(0);
					APSADCOutput_DN(0)                  <= APSADCConfigReg_DP.RampShortReset_S;
				end if;

			when APSADCCONFIG_PARAM_ADDRESSES.ADCTestMode_S =>
				-- Allow read/write of parameter only on chips which support it.
				if CHIP_APS_HAS_INTEGRATED_ADC = '1' then
					APSADCConfigReg_DN.ADCTestMode_S <= ConfigParamInput_DI(0);
					APSADCOutput_DN(0)               <= APSADCConfigReg_DP.ADCTestMode_S;
				end if;
				
			when APSADCCONFIG_PARAM_ADDRESSES.ROI0Enabled_S =>
				APSADCConfigReg_DN.ROI0Enabled_S <= ConfigParamInput_DI(0);
				APSADCOutput_DN(0)               <= APSADCConfigReg_DP.ROI0Enabled_S;
			
			when APSADCCONFIG_PARAM_ADDRESSES.ROI1Enabled_S =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.ROI1Enabled_S <= ConfigParamInput_DI(0);
					APSADCOutput_DN(0)               <= APSADCConfigReg_DP.ROI1Enabled_S;
				end if;
				
			when APSADCCONFIG_PARAM_ADDRESSES.ROI2Enabled_S =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.ROI2Enabled_S <= ConfigParamInput_DI(0);
					APSADCOutput_DN(0)               <= APSADCConfigReg_DP.ROI2Enabled_S;
				end if;
				
			when APSADCCONFIG_PARAM_ADDRESSES.ROI3Enabled_S =>
				if ENABLE_QUAD_ROI = true then
					APSADCConfigReg_DN.ROI3Enabled_S <= ConfigParamInput_DI(0);
					APSADCOutput_DN(0)               <= APSADCConfigReg_DP.ROI3Enabled_S;
				end if;
