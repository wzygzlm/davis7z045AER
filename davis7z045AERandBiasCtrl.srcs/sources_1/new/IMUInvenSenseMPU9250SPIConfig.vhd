...
entity IMUInvenSenseMPU9250SPIConfig is
	generic(
...
end entity IMUInvenSenseMPU9250SPIConfig;

architecture Behavioral of IMUInvenSenseMPU9250SPIConfig is
...
begin
	...
	begin
		...
		case ConfigParamAddress_DI is
			...
			when IMU_CONFIG_PARAM_ADDRESSES.AccelDLPF_D =>
				IMUConfigReg_DN.AccelDLPF_D                <= unsigned(ConfigParamInput_DI(tIMUConfig.AccelDLPF_D'range));
				IMUOutput_DN(tIMUConfig.AccelDLPF_D'range) <= std_logic_vector(IMUConfigReg_DP.AccelDLPF_D);

			when IMU_CONFIG_PARAM_ADDRESSES.GyroDLPF_D =>
				IMUConfigReg_DN.GyroDLPF_D                <= unsigned(ConfigParamInput_DI(tIMUConfig.GyroDLPF_D'range));
				IMUOutput_DN(tIMUConfig.GyroDLPF_D'range) <= std_logic_vector(IMUConfigReg_DP.GyroDLPF_D);
