...
architecture Behavioral of APSADCStateMachineExternal is
	...
	type tColumnState is (stIdle, stWaitADCStartup, stStartFrame, stEndFrame, stWaitFrameInterval, stColSRFeedA0, stColSRFeedA0Tick, stColSRFeedA1, stColSRFeedA1Tick, stRSFeedTick, stRSReset, stRSSwitchToReadA, stRSReadA, stRSSwitchToReadB, stRSReadB, stGSReset, stGSReadA, stGSReadB, stGSSwitchToReadA,
	                      ...
	                      stExposureValue0, stExposureValue1, stExposureValue2, stGSStartExposureTX, stRSStartExposureEvent, stRSEndExposureEvent, stGSStartExposureEvent, stGSEndExposureEvent);
	attribute syn_enum_encoding of tColumnState : type is "onehot";

	-- present and next state
	signal ColState_DP, ColState_DN : tColumnState;

	constant EXTERNAL_ADC_STARTUP_CYCLES      : integer := integer(ADC_CLOCK_FREQ * 15.0); -- At ADC_CLOCK, wait 15 microseconds.
	constant EXTERNAL_ADC_STARTUP_CYCLES_SIZE : integer := SizeCountUpToN(EXTERNAL_ADC_STARTUP_CYCLES);
	...
	constant APS_RESETTIME                   : integer := integer(ADC_CLOCK_FREQ * 2.0);
	...
	constant APS_COLSETTLETIME                       : integer := integer(ADC_CLOCK_FREQ);
	constant APS_COLSETTLETIME_SIZE                  : integer := SizeCountUpToN(APS_COLSETTLETIME);
...
begin
	...
	begin
		...
		case ColState_DP is
			...
			when stWaitADCStartup =>
				...
				if ExternalADCStartupDone_S then
					ColState_DN <= stStartFrame;
				end if;
	...
	columnSettleTimeCounter : entity work.ContinuousCounter
		...
		port map(
			...
			DataLimit_DI => to_unsigned(APS_COLSETTLETIME, APS_COLSETTLETIME_SIZE),
	...
	adcStartupCounter : entity work.ContinuousCounter
		...
		port map(
			...
			DataLimit_DI => to_unsigned(EXTERNAL_ADC_STARTUP_CYCLES, EXTERNAL_ADC_STARTUP_CYCLES_SIZE),
