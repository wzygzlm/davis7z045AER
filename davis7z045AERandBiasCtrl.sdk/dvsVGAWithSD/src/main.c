...
unsigned char picture1[8*6*3]={0};
static FATFS fatfs;
...
//	}
	...
	u32 previousETFConfigStatus = 	XGpio_DiscreteRead(&Gpio, 2) & 0x01;
	xil_printf("Current config switch status is : %d.\r\n", 	previousETFConfigStatus);
	while(1)
	{
		...
		if(currentETFConfigStatus != previousETFConfigStatus)
		{
			...
			if(currentETFConfigStatus == 0)
			{
				...
				uint8_t *sdData = (uint8_t *)malloc(1000);
			    SD_Transfer_read("bias",(u32)sdData,1000);

				u32 storeSdDataPtr = sdData;
				int index = 0;
				while(index <= 38)
				{
					char *tmpStr = (char *)malloc(10);
					for(int j = 0; j < 10; j++)
					{
						tmpStr[j] = *sdData++;
						if(tmpStr[j] == 0x2C)
						{
							tmpStr[j] = 0;
							BiasConfig[index] = strtol( tmpStr, 0, 0);
//							xil_printf("%s\n\r", tmpStr);
							index++;
							break;
						}
...
//			dvsSPIWrite(&SpiInstance, 1, 3, ~currentETFConfigStatus);
			...
			for (int a = 0; a < 37; a = a + 1 )
			{
				if(a < 5 || (a > 7 && a < 28) || a > 33)
				{
					Status = dvsSPIWrite(&SpiInstance, 5, a, BiasConfig[a]);
					if (Status != XST_SUCCESS) {
						xil_printf("Loading new configuration failed.\r\n");
						return XST_FAILURE;
					}
				}
			}
			for(int a = 0; a < 37; a++)
			{
				...
				{
					int retData;
					dvsSPIRead(&SpiInstance, 5, a, &retData);
					printf("New davis346 bias values --- Bias%d is 0x%x.\n\r", a, retData);
				}
			}
			XEventstreamtoconstencntframestream_Set_configRegs_V(&etf_inst, BiasConfig[38]);
		}
	}
    cleanup_platform();
    return 0;
