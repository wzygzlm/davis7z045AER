...
begin
	...
	LatchDVSAERReg_S <= '1' when (ConfigModuleAddress_DI = DVS_CONFIG_MODULE_ADDRESS) else '0';
	...
	begin
		...
		case ConfigParamAddress_DI is
			...
			when DVS_CONFIG_PARAM_ADDRESSES.ExternalAERControl_S =>
				DVSAERConfigReg_DN.ExternalAERControl_S <= ConfigParamInput_DI(0);
				DVSAEROutput_DN(0)                      <= DVSAERConfigReg_DP.ExternalAERControl_S;

			when DVS_CONFIG_PARAM_ADDRESSES.HasPixelFilter_S =>
				if ENABLE_PIXEL_FILTERING then
					DVSAERConfigReg_DN.HasPixelFilter_S <= '1';
					DVSAEROutput_DN(0)                  <= '1';
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel0Row_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel0Column_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel1Row_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel1Column_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel2Row_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel2Column_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel3Row_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel3Column_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel4Row_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel4Column_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel5Row_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel5Column_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel6Row_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel6Column_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel7Row_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterPixel7Column_D =>
				if ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.HasBackgroundActivityFilter_S =>
				if ENABLE_BA_REFR_FILTERING then
					DVSAERConfigReg_DN.HasBackgroundActivityFilter_S <= '1';
					DVSAEROutput_DN(0)                               <= '1';
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.FilterBackgroundActivity_S =>
				if ENABLE_BA_REFR_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterBackgroundActivityTime_D =>
				if ENABLE_BA_REFR_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterRefractoryPeriod_S =>
				if ENABLE_BA_REFR_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterRefractoryPeriodTime_D =>
				if ENABLE_BA_REFR_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.HasROIFilter_S =>
				if ENABLE_ROI_FILTERING then
					DVSAERConfigReg_DN.HasROIFilter_S <= '1';
					DVSAEROutput_DN(0)                <= '1';
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.FilterROIStartColumn_D =>
				if ENABLE_ROI_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterROIStartRow_D =>
				if ENABLE_ROI_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterROIEndColumn_D =>
				if ENABLE_ROI_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.FilterROIEndRow_D =>
				if ENABLE_ROI_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.HasSkipFilter_S =>
				if ENABLE_SKIP_FILTERING then
					DVSAERConfigReg_DN.HasSkipFilter_S <= '1';
					DVSAEROutput_DN(0)                 <= '1';
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.FilterSkipEvents_S =>
				if ENABLE_SKIP_FILTERING then
					DVSAERConfigReg_DN.FilterSkipEvents_S <= ConfigParamInput_DI(0);
					DVSAEROutput_DN(0)                    <= DVSAERConfigReg_DP.FilterSkipEvents_S;
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.FilterSkipEventsEvery_D =>
				if ENABLE_SKIP_FILTERING then
					DVSAERConfigReg_DN.FilterSkipEventsEvery_D                   <= unsigned(ConfigParamInput_DI(tDVSAERConfig.FilterSkipEventsEvery_D'range));
					DVSAEROutput_DN(tDVSAERConfig.FilterSkipEventsEvery_D'range) <= std_logic_vector(DVSAERConfigReg_DP.FilterSkipEventsEvery_D);
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.HasPolarityFilter_S =>
				if ENABLE_POLARITY_FILTERING then
					DVSAERConfigReg_DN.HasPolarityFilter_S <= '1';
					DVSAEROutput_DN(0)                     <= '1';
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.FilterPolarityFlatten_S =>
				if ENABLE_POLARITY_FILTERING then
					DVSAERConfigReg_DN.FilterPolarityFlatten_S <= ConfigParamInput_DI(0);
					DVSAEROutput_DN(0)                         <= DVSAERConfigReg_DP.FilterPolarityFlatten_S;
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.FilterPolaritySuppress_S =>
				if ENABLE_POLARITY_FILTERING then
					DVSAERConfigReg_DN.FilterPolaritySuppress_S <= ConfigParamInput_DI(0);
					DVSAEROutput_DN(0)                          <= DVSAERConfigReg_DP.FilterPolaritySuppress_S;
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.FilterPolaritySuppressType_S =>
				if ENABLE_POLARITY_FILTERING then
					DVSAERConfigReg_DN.FilterPolaritySuppressType_S <= ConfigParamInput_DI(0);
					DVSAEROutput_DN(0)                              <= DVSAERConfigReg_DP.FilterPolaritySuppressType_S;
				end if;
			...
			when DVS_CONFIG_PARAM_ADDRESSES.StatisticsEventsRow64_D =>
				if ENABLE_STATISTICS then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.StatisticsEventsRow32_D =>
				if ENABLE_STATISTICS then
					DVSAEROutput_DN(31 downto 0) <= std_logic_vector(DVSAERConfigInfoOut_DI.StatisticsEventsRow_D(31 downto 0));
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.StatisticsEventsColumn64_D =>
				if ENABLE_STATISTICS then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.StatisticsEventsColumn32_D =>
				if ENABLE_STATISTICS then
					DVSAEROutput_DN(31 downto 0) <= std_logic_vector(DVSAERConfigInfoOut_DI.StatisticsEventsColumn_D(31 downto 0));
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.StatisticsEventsDropped64_D =>
				if ENABLE_STATISTICS then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.StatisticsEventsDropped32_D =>
				if ENABLE_STATISTICS then
					DVSAEROutput_DN(31 downto 0) <= std_logic_vector(DVSAERConfigInfoOut_DI.StatisticsEventsDropped_D(31 downto 0));
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.StatisticsFilteredPixels64_D =>
				if ENABLE_STATISTICS and ENABLE_PIXEL_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.StatisticsFilteredPixels32_D =>
				if ENABLE_STATISTICS and ENABLE_PIXEL_FILTERING then
					DVSAEROutput_DN(31 downto 0) <= std_logic_vector(DVSAERConfigInfoOut_DI.StatisticsFilteredPixels_D(31 downto 0));
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.StatisticsFilteredBackgroundActivity64_D =>
				if ENABLE_STATISTICS and ENABLE_BA_REFR_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.StatisticsFilteredBackgroundActivity32_D =>
				if ENABLE_STATISTICS and ENABLE_BA_REFR_FILTERING then
					DVSAEROutput_DN(31 downto 0) <= std_logic_vector(DVSAERConfigInfoOut_DI.StatisticsFilteredBackgroundActivity_D(31 downto 0));
				end if;

			when DVS_CONFIG_PARAM_ADDRESSES.StatisticsFilteredRefractoryPeriod64_D =>
				if ENABLE_STATISTICS and ENABLE_BA_REFR_FILTERING then
			...
			when DVS_CONFIG_PARAM_ADDRESSES.StatisticsFilteredRefractoryPeriod32_D =>
				if ENABLE_STATISTICS and ENABLE_BA_REFR_FILTERING then
